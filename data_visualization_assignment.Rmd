...
# The distribution is skewed to the right suggesting that there are a large number of genes that are expressed at low levels. This is quite normal because expression of every gene in the genome makes cell survival impossible. This is not specific to the liver because this concept is universal. While the distribution of genes that are highly expressed might be different, the general shape of the data is the same.
```
...
```{r q2}
ggplot(dt, aes(x = `Liver`)) + 
  geom_density(breaks = seq(0, 100, 2), position = "stack", color = "red") +
  labs(
    x = 'TPM',
    y= 'Gene Expression'
  ) +
...
ggplot(dt, aes(x = `Liver` + 1, y = `Heart - Left Ventricle` + 1)) + 
  geom_point(alpha = .2, size = 1, color = "red") +
  ...
  labs(
    x= "Liver Expression",
    y= "Heart Expression",
    title= 'Liver vs Heart Expression'
  )
...
ggplot(mit, aes(x = `Liver` + 1, y = `Heart - Left Ventricle` + 1, color = is_mitochondrial)) + 
...
ggplot(dt_long, aes(x = TPM)) + 
  geom_histogram(breaks = seq(0, 100, 2), position = "stack", fill = "blue") +
  labs(
    x = 'TPM',
    y= 'Gene Expression'
  )+
